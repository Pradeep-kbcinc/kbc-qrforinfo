<template>
  <div>
    <v-img height="300" cover src="@/assets/bgHome.jpeg" class="pa-10">
      <v-row>
        <v-col cols="12" md="6">
          <h2 class="text-h4 font-weight-bold">Find Your Perfect Property</h2>
          <p class="mt-4 text-h6 text-blue-grey-darken-1">
            Manage listings, connect with buyers, and discover premium real estate opportunities.
          </p>

          <div class="mt-4">
            <v-btn @click="router.push('/properties')" height="38" :width="mobile ? '140' : ''" rounded="lg"
              class="elevation-0 text-none font-weight-bold" color="info">
              <v-icon>mdi-file-sign</v-icon> View Properties
            </v-btn>

            <v-btn @click="router.push('/add-new-property')" height="38" :width="mobile ? '140' : ''" rounded="lg"
              class="elevation-0 text-none font-weight-bold mx-2" color="success">
              <v-icon>mdi-plus</v-icon> Add Property
            </v-btn>

            <!-- <v-btn variant="outlined" @click="router.push('/add-new-property')" height="38" :width="mobile ? '140' : ''" rounded="lg"
              class="elevation-0 text-none font-weight-bold bg-white" color="purple">
              <v-icon>mdi-plus</v-icon> Explore Listings
            </v-btn> -->
          </div>
        </v-col>
        <v-col cols="12" md="6">
    ...
    <v-container fluid>
      <!-- <div class="mt-1 d-flex align-center">
        <div>
          ...
          <p>Welcome Back, <span class="text-primary">{{ authStore?.userDetails?.FNAME }} {{
            authStore?.userDetails?.MNAME }} {{ authStore?.userDetails?.LNAME }}</span></p>
        </div>
      ...
      <section class="py-4">
        <v-row>
          <v-col cols="6" md="3">
            <v-card class="d-flex align-center justify-space-between rounded-lg card-box-shadow" height="100"
          ...
          <v-col cols="6" md="3">
            <v-card @click="openDraftList" class="rounded-lg card-box-shadow">
              ...
              <v-card-text class="text-center text-grey-darken-1 mb-1 text-subtitle-1 font-weight-medium">
                <p>Drafts</p>
                <h5 class="text-lightBlack text-h4 font-weight-bold mt-1">{{ statisticsData[1]?.METRIC_VALUE || 0 }}
                </h5>
              </v-card-text>
            </v-card>
          </v-col>
        ...
        <v-card class="card-box-shadow rounded-lg">
          <v-card-text class="">
            <div class="d-md-flex px-2 mb-2 align-center">
              ...
              <v-btn @click="historyModal = true" class="elevation-0 text-none font-weight-bold mr-2" rounded="lg"
                color="grey" height="38" :width="mobile ? '160' : ''">My Scanned History</v-btn>
              <v-btn @click="openDraftList" height="38" :width="mobile ? '140' : ''" rounded="lg"
            ...
            <div v-else-if="!isLoading && propertyArr && propertyArr.length == 0"
    ...
    <v-dialog max-width="800" v-model="uploadImageModal">
      ...
      <v-card rounded="t-0 b-lg">
        <v-card-text>
            ...
            <div>
              <v-row v-if="selectedData.IMAGES && selectedData.IMAGES.length > 0">
                <v-col cols="3" v-for="(item, ind) in selectedData.IMAGES" :key="ind">
                  <v-img height="200" @click="deleteImage(item, ind)" lazy-src="@/assets/property_placeholder.webp"
                    contain class="rounded-lg" :src="item.IMAGE_URL">
          ...
          <v-file-upload density="compact" v-model="addedImagesArr" title="Browse Or Drag and Drop Here" clearable
            multiple></v-file-upload>
    ...
    <v-dialog max-width="800" v-model="historyModal">
      ...
      <v-card rounded="b-lg t-0" min-height="400">
        ...
        <div v-else> -->
          ...
          <div class="d-flex">
            <v-date-input persistent-placeholder class="rounded-lg" v-model="selectedDate" density="compact"
              label="Date input" variant="outlined"></v-date-input>
            <v-btn :loading="historyLoader" :disabled="!selectedDate" @click="fetchHistory()" color="primary"
              elevation="0" class="text-none rounded-lg ml-2" height="40"><v-icon class="mt-1">mdi-magnify</v-icon>
              Search</v-btn>
          </div>
          ...
          <div v-for="(propertyObj, i) in historyArr" :key="i" class="rounded-lg">
            <v-list-item class="px-2 my-2 pointer">
              ...
              <template #append>
                <div class="text-end">
                  <v-btn @click.stop="router.push(`/property/${propertyObj.PROPERTY_ID}?createdBy=false`)" rounded="lg"
                    class="text-none font-weight-bold text-subtitle-1" color="success" elevation="0">
